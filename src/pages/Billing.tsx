...
export default function Billing() {
  ...
  const handleUpgrade = async () => {
    ...
    try {
      ...
      razor.on('payment.failed', () => toast.error('Payment failed. Please try again.'));
      razor.open();
    } catch (error: any) {
  ...
  const handleStripeCheckout = async () => {
    ...
    try {
      ...
      if (!resp.ok || !data.url) throw new Error('Stripe session could not be created');
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="rounded-3xl border border-orange-500/40 bg-orange-500/10 p-6">
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            <button onClick={handleUpgrade} disabled={loading || isPro} className="px-5 py-3 rounded-xl bg-orange-500 disabled:opacity-50">{loading ? 'Processing...' : 'Pay with Razorpay'}</button>
            <button onClick={handleStripeCheckout} disabled={loading || isPro} className="px-5 py-3 rounded-xl border border-white/20 bg-white/10 disabled:opacity-50">Pay with Stripe</button>
          </div>
        </div>
